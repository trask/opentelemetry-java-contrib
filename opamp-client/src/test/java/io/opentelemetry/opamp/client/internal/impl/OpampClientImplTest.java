...
class OpampClientImplTest {
  ...
  /**
   * Test that verifies the remote config status setter works correctly. This test was failing
   * sporadically with "Expecting actual not to be null" error due to potential race conditions in
   * the original implementation.
   */
  @Test
  void verifyRemoteConfigStatusSetter() {
    // Create a RemoteConfigStatus object to set
    RemoteConfigStatus expectedStatus =
  ...
  void verifyRemoteConfigStatusSetter_withNullValue() {
    ...
    RemoteConfigStatus actualStatus = client.getRemoteConfigStatus();
    assertThat(actualStatus).as("Remote config status should be null when set to null").isNull();
  }

  @Test
  void verifyRemoteConfigStatusSetter_multipleUpdates() {
    // Create first status
    RemoteConfigStatus firstStatus =
        new RemoteConfigStatus.Builder()
            .status(RemoteConfigStatuses.RemoteConfigStatuses_APPLIED)
            .build();

    // Create second status
    RemoteConfigStatus secondStatus =
        new RemoteConfigStatus.Builder()
  ...
  /**
   * Test that attempts to reproduce race conditions by rapidly setting and getting the status. This
   * test runs the setter multiple times to try to catch sporadic failures.
   */
  @Test
  ...
  /**
   * Multi-threaded test to verify thread safety of setRemoteConfigStatus. This test attempts to
   * reproduce the sporadic null pointer issues by accessing the status concurrently from multiple
   * threads.
   */
  @Test
              ...
              () -> {
                try {
                  ...
                  for (int j = 0; j < operationsPerThread; j++) {
                    ...
                    } else if (!RemoteConfigStatuses.RemoteConfigStatuses_APPLIED.equals(
                        actualStatus.status)) {
  ...
  void verifyRemoteConfigStatusSetter_concurrentDifferentValues() throws InterruptedException {
              ...
              () -> {
                try {
                  ...
                  for (int j = 0; j < 100; j++) {
                    ...
                    assertThat(actualStatus)
                        .as(
                            "Status should never be null in thread "
                                + threadIndex
                                + " iteration "
                                + j)
                        .isNotNull();
    ...
    assertThat(client.getRemoteConfigStatus()).as("Final status should not be null").isNotNull();
  }
}
